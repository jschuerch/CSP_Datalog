...
pyDatalog.create_terms('datapath, A, Z1, Z2, D, D2')

knows(X,Y,Z) <= called(X,Y,Z)# & (Z >= date_board_decision) & (Z <= date_shares_bought)
knows(X,Y,Z) <= texted(X,Y,Z)# & (Z >= date_board_decision) & (Z <= date_shares_bought)
#print (knows(suspect,Y,Z))
#print (knows(company_Board[1],Y,Z))
...
## Jenny
datapath(X,Y,Z,P,D) <= (datapath(X,A,Z2,P2,D2)
                                & knows(A,Y,Z)
                                & (Z>=Z2)
                                & X._not_in(P2)
                                & A._not_in(P2)
                                & Y._not_in(P2)
                                & (X!=Y)
                                & (Z >= date_board_decision)
                                & (Z <= date_shares_bought)
                                & (P==P2 + [A])#[Z2, A, Z])
                                & (D==D2 + [Z2])
                                )
datapath(X,Y,Z,P,D) <= (knows(X,Y,Z)
                                & (Z >= date_board_decision)
                                & (Z <= date_shares_bought)
                                & (P==[])
                                & (D==[])
                                )
print(datapath(X, suspect, Z, P, D) & X._in(company_Board))
...
# Task 6: Find all the communication paths that lead to the suspect (with the restriction that the dates have to be ordered correctly)
get_dated_paths(X, Y, P, D, PD) <= (get_dated_paths(X, Z, P2, D2, PD2)
                                & knows(Z, Y, D)
                                & (X != Y)
                                & X._not_in(P2)
                                & Y._not_in(P2)
...
get_dated_paths(X, Y, P, D, PD) <= (knows(X, Y, D)
                                & (P == [])
                                & (PD == [])
                                & (D >= date_board_decision)
                                & (D <= date_shares_bought))
print(get_dated_paths(X, suspect, P, D, PD) & X.in_(company_Board))
